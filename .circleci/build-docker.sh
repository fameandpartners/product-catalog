...
docker build --build-arg ConnectionStrings__FameConnection=$ConnectionStrings__FameConnection --build-arg CIRCLE_BUILD_NUM=$CIRCLE_BUILD_NUM --build-arg CIRCLE_BRANCH=$CIRCLE_BRANCH --build-arg CIRCLE_SHA1=$CIRCLE_SHA1 -t $NAME:$VERSION .
...
aws elasticbeanstalk create-application-version --application-name "Product Catalog" \
